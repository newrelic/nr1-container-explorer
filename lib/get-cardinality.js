...
export default async function getCardinality(props) {
  let { accountIds, eventType, where, timeWindow } = props;

  eventType = quote(eventType);
  where = where || 'true';
  timeWindow = timeWindow || 'SINCE 1 hour ago';
  let keySet = await nrdbQuery(
    accountIds,
  ...
  const batchSize = 100;
  const facets = [];
  for (let i = 0; i < keySet.length; i += batchSize) {
    const batch = keySet.slice(i, i + batchSize);
    const select = batch.map(
      (key) => `uniqueCount(${quote(key.key)}) AS '${key.key}'`
    );
    const nrql = `SELECT ${select} FROM ${eventType} WHERE ${where} ${timeWindow}`;

    const uniqueCounts = (await nrdbQuery(accountIds, nrql))[0];
